   ...
   */
    ...
    */
    function clickAll(isPositive){
      ...
      var conf_str = false;
        conf_str = confirm("小可爱，你即将执行“" + isPositiveMesg + "全部资源”操作，如果资源量较大（> 1000），耗时就会较久，打开的页面也会较多哦！不过都会自动关闭的哦！！！" + "\n\n" + "小可爱，资源较多时，还请三思啊！！！" + "\n\n" + "你是否要执行？");
       if(conf_str){
        var list = document.getElementsByClassName("res-row-open-enable");
        var succNum = 0;
        var failNum = 0;
...
//     background-color: #4d79ff !important;
  ...
  //#refresh{ background:rgba(204, 0, 0,1);}
</style>`;
     ...
     <div id="module-3">\
       <div class="clear30"></div>\
        <div class="res-row-title" >\
           ...
           <span class="span-display" style="color: #0BD" > | (范围：1 至 20；不在 [1,20] 该区间内 / 不填写 则视为全选)</span>\
 ...
 */
     ...
     * For CRX
     */
    //send message to bg.js to create Tab
    function sendMessOnDownload(){
      // chrome.tabs.create({url:"chrome://settings/?search=downloads"});
      chrome.runtime.sendMessage({createTab: "onDownload"});
    }
    function sendMessOffDownload(){
      // chrome.tabs.create({url:"chrome://settings/?search=downloads"});
      chrome.runtime.sendMessage({createTab: "offDownload"});
    }
